...
import { EcommerceOrder, WhatsappWaba, WhatsappPhoneNumber } from "../models/index.js";
...
const buildOrderItemsFromPayload = (payload) => {
  const quantity = Number(payload.qty ?? payload.quantity ?? 1) || 1;
  const totalAmount = Number(payload.totalAmount ?? payload.amount ?? 0) || 0;
  const unitPrice = quantity > 0 ? totalAmount / quantity : totalAmount;

  return [{
    product_retailer_id: payload.productRetailerId || payload.sku || payload.orderNumber || "webhook-item",
    quantity,
    price: unitPrice,
    name: payload.description || payload.itemName || payload.productName || "Webhook item",
    raw: payload
  }];
};

const upsertOrderFromWebhookPayload = async ({
  userId,
  contactId,
  ...
  const orderNumber = payload.orderNumber || payload.order_number || payload.orderId || payload.id;
  const waMessageId = `ecommerce-webhook:${webhookToken}:${orderNumber || crypto.createHash("sha1").update(JSON.stringify(payload)).digest("hex")}`;

  return await EcommerceOrder.findOneAndUpdate(
    {
      user_id: userId,
    ...
    },
    {
      $set: {
        ...
        wa_message_id: waMessageId,
        wa_order_id: orderNumber || null,
        currency: payload.currency || payload.currencyCode || "EGP",
        total_price: payload.totalAmount ?? payload.amount ?? null,
        status: "first_message",
        items: buildOrderItemsFromPayload(payload),
        raw_payload: payload,
        phone_no_id: whatsappPhoneNumberId,
        deleted_at: null
      }
    },
    {
...
const startOrderAutomationContext = async ({
  userId,
  payload,
  webhook,
  order,
  ...
  const flows = await AutomationFlow.find({
    ...
    is_active: true,
    deleted_at: null,
    "triggers.event_type": "order_received"
  }).populate("user_id");

  for (const flow of flows) {
    const startNode = automationEngine.getStartNodes(flow)[0];
    ...
    const inputData = {
      ...
      wa_order_id: payload.orderNumber,
    ...
    await automationEngine.executeFlow(flow, inputData);
  }
};
...
export const triggerWebhook = async (req, res) => {
  try {
    ...
    if (!contact) {
      const contactName =
        Webhook.getNestedValue(payload, "customerName") ||
    ...
    if (wabaId) {
      whatsappPhoneNumber = await WhatsappPhoneNumber.findOne({
        ...
        .populate("waba_id")
        .sort({ is_primary: -1, last_used_at: 1 })
        .lean();
    }
    ...
    if (!whatsappPhoneNumber || !whatsappPhoneNumber.waba_id) {
      ...
      if (whatsappPhoneNumber?.waba_id?._id && String(template.waba_id || "") !== String(whatsappPhoneNumber.waba_id._id)) {
        await Template.findByIdAndUpdate(template._id, {
          waba_id: whatsappPhoneNumber.waba_id._id
        });
        template.waba_id = whatsappPhoneNumber.waba_id._id;
      }
    }
    ...
    if (!whatsappPhoneNumber || !whatsappPhoneNumber.waba_id) {
      ...
      await webhook.save();
      return res.status(400).json({
        error: "No active WhatsApp phone number found for this webhook user.",
        template_waba_id: wabaId || null
      });
    }
    ...
    const order = await upsertOrderFromWebhookPayload({
      userId,
    ...
    try {
      ...
      await startOrderAutomationContext({
        userId,
        payload,
        webhook,
        order,
