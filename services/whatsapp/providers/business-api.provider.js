...
export default class BusinessAPIProvider extends BaseProvider {
  ...
  async sendWhatsAppAPIMessage(params) {
    ...
    if (!response.ok) {
      ...
      const details = error.error_data?.details || error.message || response.statusText || 'Unknown error';
      const code = error.code ? ` code=${error.code}` : '';
      const subcode = error.error_subcode ? ` subcode=${error.error_subcode}` : '';
      const fbtrace = error.fbtrace_id ? ` fbtrace_id=${error.fbtrace_id}` : '';
      throw new Error(
        `WhatsApp API error:${code}${subcode}${fbtrace} ${details}`
      );
    }
