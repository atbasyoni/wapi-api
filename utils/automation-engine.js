...
class AutomationEngine {
  ...
  async handleMessageReceived(eventData) {
    try {
      ...
      const normalizedMessagePayload = eventData.messagePayload || this.normalizeMessagePayload(message);
        ...
        if (flow && flow.is_active && !flow.deleted_at) {
          ...
          if (shouldExecute) {
            ...
            await this.executeFlow(flow, {
              ...
              interactiveId: eventData.interactive_id || eventData.interactive_reply_id || null,
              interactive_id: eventData.interactive_id || eventData.interactive_reply_id || null,
              interactiveReplyId: eventData.interactive_reply_id || eventData.interactive_id || null,
  ...
  getNestedValue(obj, path) {
    ...
    if (path === 'sender.phone') {
      return obj.sender?.phone || obj.senderNumber || obj.senderPhone || obj.contact?.phone_number;
    }
    if (path === 'sender.name') {
      return obj.sender?.name || obj.senderName || obj.contact?.name || obj.senderNumber || 'Customer';
    }
  ...
  async resumeExecution(flow, execution, eventData, messagePayload) {
    ...
    const senderPhone = eventData.senderNumber || execution.input_data?.senderNumber || execution.contact_identifier || '';
    const senderName = eventData.senderName || eventData.contact?.name || execution.input_data?.contact?.name || senderPhone || 'Customer';
